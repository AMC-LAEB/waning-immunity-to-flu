...
# Subset so that only individuals 1039 are plotted
...
fig_sero<-ggplot()+
  ...
  guides(pattern = guide_legend(override.aes = list(fill = "white",pattern_color='black')),
...
# Get legend
leg = get_legend(fig_sero) 

#Arrange plots
p = ((((fig_epi/fig_gmt/fig_wane)|(fig_sero & theme(legend.position='none'))) + plot_layout(widths=c(0.57,0.43)))
     /as_ggplot(leg)) + plot_layout(heights=c(0.5,0.02))


out_name<-paste0("figs/SuppFig_5.pdf")
