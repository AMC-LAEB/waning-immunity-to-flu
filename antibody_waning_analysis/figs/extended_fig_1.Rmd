...
fig1<-ggplot(acs_dat, aes(x=date, y=LogTiter, color=strain, linetype=as.factor(rep)))+
  ...
  labs(x="Date", y=expression(paste(Log[2], " titre")), color="Strain", linetype="Replicate")+
  facet_wrap(vars(pat))
...
vis_dat<-vis_dat%>%group_by(pat, year, date, strain)%>%summarise(LogTiter=mean(LogTiter, na.rm=T))
...
fig2<-ggplot(vis_dat, aes(x=year, y=LogTiter, group=pat, color=strain))+
  ...
  scale_y_continuous(breaks = 0:8)+
  scale_x_continuous(breaks=2020:2021, limits = c(2019.8, 2021.2))+
  scale_color_manual(values=c("#44AA99", "#DDCC77", "#AA4499", "#332288"))+
  labs(x="Date", y=expression(paste(Log[2], " titre")), color="Strain", linetype="Replicate")+
...
ggsave("extended_ACS_data_fig_1.pdf", fig, width= 183, height=247, units="mm")
```
