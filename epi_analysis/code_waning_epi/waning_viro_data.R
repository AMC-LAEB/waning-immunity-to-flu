...
#File path
epi <- read.csv("viro_raw.csv")
#Threshold for the percentage of total circulation in a season for a subtype to be considered dominant
thresh = 0.3
...
for (country in unique(byyear$Country)){
  minyear = 2019 - length(which(sort(unique(byyear[byyear$Country==country,]$Season),decreasing=T) - seq(2019,2019-nrow(byyear[byyear$Country==country,])+1,-1) == 0)) + 1
...
#write.csv(byyear,"viro_data.csv",row.names=F,quote=F)
