...
plot_posteriors_size <- function(){
  ...
  offset = 3
  legend(y = 1.55,x=2.1+offset,pch = c(19,18,15),title="",legend=c("","",""),pt.cex=c(0.8,1.2,1)*1.5,y.intersp=0.7,x.intersp=0,cex=1,horiz=T,bty="n",bg="transparent")
  text(y=1.4,x=3+offset,lab="Models\nwithout\nseason\neffects")

  legend(y = 1.55,x=.5+offset,pch = c(1,5,0),title="",legend=c("","", ""),pt.cex=c(0.8,0.8,1)*1.5,y.intersp=0.7,x.intersp=0,horiz=T,cex=1,bty="n",bg="transparent")
  text(y=1.4,x=1.45+offset,lab="Models\nwith\nseason\neffects")
  
  legend(y = 1.72,x=3.7+offset,pch = c(19,19,19),title="",legend=c("A/H3N2","A/H1N1pdm09","B"),col=c("#44AA99","#DDCC77","#AA4499"),pt.cex=c(1,1,1)*1.5,horiz=F,y.intersp=1,x.intersp=0.8,cex=1,bty="n",bg="transparent")

  legend(y = 1.72,x=5.4+offset,pch=c(1,5,0),title="",legend=c("Seasons since substantial circulation","Previous season size","Sum two previous seasons size"),pt.cex=c(1,1,1)*1.5,horiz=F,y.intersp=1,x.intersp=0.8,cex=1,bty="n",bg="transparent")
  
  
  

  
  segments(4.5,-1.1,14.5,-1.1)
  segments(4.5,-1.1,4.5,-1.05)
  segments(14.5,-1.1,14.5,-1.05)
  
  text(y=-1.23,x=9.5,label="Season effects")
  
}

setWidth = 180*0.039370 
setFontSize = 5
